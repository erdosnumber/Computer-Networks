...
estimated_rtt_list=[[] for i in range(num_threads)]
dev_rtt_list=[[] for i in range(num_threads)]
timeout_list=[[] for i in range(num_threads)]

def get_lines (i, offset, limit, rate, ai):
    ...
    while (total_received<limit):
        ...
        rtt=ending_time-sending_time
        new_estimated_rtt=0.875*estimated_rtt+0.125*rtt
        new_dev_rtt=0.75*dev_rtt+0.25*abs(rtt-estimated_rtt)
        estimated_rtt=new_estimated_rtt
        dev_rtt=new_dev_rtt

        timeout=estimated_rtt+4*dev_rtt

        # print("Thread: ",i," Timeout: ",timeout)
        ...
        estimated_rtt_list[i].append(estimated_rtt)
        dev_rtt_list[i].append(dev_rtt)
        timeout_list[i].append(timeout)

        #print(i, rate, estimated_rtt, dev_rtt, timeout)
        rate += ai
...
for i in range(num_threads):
    ...
    thread = threading.Thread(target=get_lines, args=(i, pref[i], pref[i + 1], rate, ai))
...
# for i in range(num_threads):
#     rate_list[i] = rate_list[i][20: ]
#     time_list[i] = time_list[i][20: ]
#     estimated_rtt_list[i] = estimated_rtt_list[i][20: ]
#     dev_rtt_list[i] = dev_rtt_list[i][20: ]
#     timeout_list[i] = timeout_list[i][20: ]
...
# plt.legend()
# plt.savefig("week3_rate.png")

# fig=plt.figure()
# for i in range(num_threads):
#     plt.plot(time_list[i],estimated_rtt_list[i],color=colors[i],label="Thread "+str(i))
#     plt.xlabel("Time elapsed")
#     plt.ylabel("Estimated RTT")
# plt.legend()
# plt.savefig("week3_estimated_rtt.png")

# fig=plt.figure()
# for i in range(num_threads):
#     plt.plot(time_list[i],dev_rtt_list[i],color=colors[i],label="Thread "+str(i))
#     plt.xlabel("Time elapsed")
#     plt.ylabel("Deviation in RTT")
# plt.legend()
# plt.savefig("week3_dev_rtt.png")

# fig=plt.figure()
# for i in range(num_threads):
#     plt.plot(time_list[i],timeout_list[i],color=colors[i],label="Thread "+str(i))
#     plt.xlabel("Time elapsed")
#     plt.ylabel("Timeout")
# plt.legend()
# plt.savefig("week3_timeout_list.png")